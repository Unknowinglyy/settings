return {
  ...
  config = function()
    ...
    -- vim.keymap.set("n", "<leader>B", ui.toggle_quick_menu, { desc = "toggle the buffer menu" })
    vim.keymap.set("n", "<leader><leader>", ui.toggle_quick_menu, { desc = "toggle the buffer menu" })

    -- let's see if I like this better
    vim.keymap.set("n", "<C-h>", ui.nav_next, { desc = "go to next buffer in the list" })
    vim.keymap.set("n", "<C-t>", ui.nav_prev, { desc = "go to prev buffer in the list" })

    -- buffer session save
    -- local file = vim.fn.stdpath("data") .. "/buffer_manager.session"
